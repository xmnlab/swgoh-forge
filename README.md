...
The wrapper starts the pinned Comlink container on an unused loopback port and requests the public `/player` profile. Because that response does not contain Galactic Power for each unit, the updater also requests the required current game-data collections and uses the pinned `swgoh_comlink` `StatCalc` implementation locally to calculate character and ship GP from actual roster progression. The same live skill definitions convert raw skill tiers to displayed ability levels and count applied zeta and omicron tiers without inferring them from names. The wrapper bootstraps or reuses `.venv-data`, writes the normalized snapshot atomically, and stops Comlink.
...
After an update, reload the site and open **Roster**. Loading a saved Ally Code resets previous calculated results and the cached Build exclusions, then excludes every character in the current catalog that is absent from that roster. The selected Ally Code is remembered locally and restored after a refresh as long as its snapshot remains in the current static build; the roster data itself is always read from that current snapshot rather than copied into browser storage. The Build optimizer therefore recommends only owned characters unless exclusions are manually changed afterward. Explicitly requiring an excluded unit restores that exact unit as a hypothetical build target. Formations across Build, simulation, missions, counters, and whole-roster results show the sum of their calculated unit GP; fleets do the same for their capital ship, starters, and reinforcements. An unactivated unit contributes 0 GP and is labeled locked; an activated unit whose legacy snapshot lacks calculated GP still makes the total unavailable. Character formations also overlay a leader-synergy ring on every ally: the ring always has one segment per modeled leader group, matched groups glow vibrant green, and unmatched groups remain dim. The caption names those groups and reports ally coverage. Avatar interiors consistently use the relic-inspired alignment palette: cyan for Light Side, crimson for Dark Side, and silver for Neutral. With a roster loaded, unactivated units keep a paler cyan, crimson, or silver alignment tint with a desaturated, dashed treatment. Owned-unit avatars in both selectors and results show an exact gear/relic badge, training level, and star count. Concentric rings increase across four gear bands while preserving the precise `G` value in the badge. Their ring colors progress through white, green, blue, purple, and Gear XII gold. Gear XIII and relic rings use the alignment palette; actual relics receive the brighter multi-ring glow. Result avatars also show ability/zeta/omicron counts, and selecting an avatar opens the full progression and unit-GP breakdown. Relic Galactic Legends receive the additional gold Legend ring, with a brighter gold treatment when Comlink reports their Ultimate as purchased.

The generated file intentionally omits the internal player ID, raw equipped-mod records, datacrons, and other fields the interface does not use. It does include the Ally Code, player name, guild name, unit ownership, progression, calculated unit GP, normalized ability levels, and applied power-up counts. Snapshots created by an older updater remain readable, but show unavailable GP totals and dashes for missing zeta or omicron counts until that Ally Code is refreshed. **Committing `data/rosters.js` publishes that information through the static site. Only commit a snapshot when the player expects it to be public.**
